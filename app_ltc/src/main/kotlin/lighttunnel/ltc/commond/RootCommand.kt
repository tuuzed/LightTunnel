...
internal class RootCommand : CliktCommand(name = ManifestUtils.appName, invokeWithoutSubcommand = true) {
    ...
    private fun startupClient() {
        val workerThreads = configJson.getOrNull("worker_threads") ?: -1
        val serverAddr = configJson.getOrNull<String>("server_addr")?.let { IpUtils.parseAddr(it) } ?: ("127.0.0.1" to 7080)
        val serverSslAddr = configJson.getOrNull<String>("server_ssl_addr")?.let { IpUtils.parseAddr(it) } ?: ("127.0.0.1" to 7443)
        val sslContext = runCatching {
            SslContextUtils.forClient(
                jks = configJson.getOrNull("ssl_jks") ?: "ltc.jks",
                storePassword = configJson.getOrNull("ssl_store_password") ?: "ltcpass",
            )
        ...
        val tunnels = configJson.getOrNull<JSONArray>("tunnels") ?: return
        tunnels.filterIsInstance<JSONObject>().forEach {
            val ssl = it.getOrNull<Boolean>("ssl") ?: false
            val useEncryption = it.getOrNull<Boolean>("use_encryption") ?: false
            val tunnelRequest = it.asTunnelRequest(configJson.getOrNull("auth_token"))
        ...
        val httpApiBindAddr = configJson.getOrNull<String>("bind_http_api_addr")?.let { IpUtils.parseAddr(it) }
        val httpApiUsername = configJson.getOrNull<String>("http_api_username")
        val httpApiPassword = configJson.getOrNull<String>("http_api_password")
    ...
    private fun configureLogger() {
        ...
        if (isStartFromCommand) {
            ...
            level = Level.toLevel(configJson.getOrNull("log_level"), Level.INFO)
