...
internal class HttpTunnelChannelHandler(
    ...
    override fun channelRead(ctx: ChannelHandlerContext, msg: Any) {
        ...
        when (msg) {
            is HttpRequest -> {
                // 插件处理
                ctx.channel().attr(AK_IS_PLUGIN_HANDLED).set(httpPlugin?.doHttpRequest(httpContext, msg))
                ...
                // 拦截器处理
                ctx.channel().attr(AK_IS_INTERCEPTOR_HANDLED)
    ...
    private val ChannelHandlerContext.isPluginHandle
        get() = this.channel().attr(AK_IS_PLUGIN_HANDLED).get() == true

    private val ChannelHandlerContext.isInterceptorHandle
        get() = this.channel().attr(AK_IS_INTERCEPTOR_HANDLED).get() == true
