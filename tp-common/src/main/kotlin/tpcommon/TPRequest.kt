...
class TPRequest private constructor(
    val type: TPType,
    ...
    override fun toString(): String {
        return when (type) {
            TPType.TCP -> "[ $localAddr:$localPort<-tcp://{server}:$remotePort { ${optionsMapToLine(options)} } ]"
            TPType.HTTP -> "[ $localAddr:$localPort<-http://$host { ${optionsMapToLine(options)} } ]"
            TPType.HTTPS -> "[ $localAddr:$localPort<-https://$host { ${optionsMapToLine(options)} } ]"
            else -> ""
        }
    }
    ...
    companion object Factory {
        ...
        fun fromBytes(bytes: ByteArray): TPRequest {
            val buffer = Unpooled.wrappedBuffer(bytes)
            try {
                val proto = TPType.ofValue(buffer.readByte())
                ...
                return TPRequest(proto, loadAddr, localPort, options)
            } catch (e: Exception) {
                throw TPException("解析失败，数据异常", e)
            } finally {
                buffer.release()
            }
        }
        ...
        fun ofTcp(
            ...
            return TPRequest(TPType.TCP, localAddr, localPort, options)
        }
        ...
        fun ofHttp(
            ...
            authToken?.also { options[AUTH_TOKEN] = authToken }
            return TPRequest(if (https) TPType.HTTPS else TPType.HTTP, localAddr, localPort, options)
        }
