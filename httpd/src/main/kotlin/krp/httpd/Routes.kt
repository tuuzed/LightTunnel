...
class Routes internal constructor() {
    ...
    private val notFoundRouteFallback: (request: FullHttpRequest) -> FullHttpResponse = {
    ...
    fun intercept(
        path: Regex,
        interceptor: (request: FullHttpRequest) -> FullHttpResponse?
    ) = lock.write { interceptors[path] = interceptor }

    fun route(
        path: Regex,
        handler: (request: FullHttpRequest) -> FullHttpResponse
    ...
    internal fun doHandle(request: FullHttpRequest): FullHttpResponse = lock.read {
        ...
        return interceptors.entries.firstOrNull { it.key.matches(path) }?.value?.invoke(request)
            ...
            ?: notFoundRouteFallback.invoke(request)
    }
