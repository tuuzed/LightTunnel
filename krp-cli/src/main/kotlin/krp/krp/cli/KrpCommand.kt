...
import krp.extensions.*
import krp.krp.Krp
import krp.krp.cli.internal.DefaultKrpListener
import krp.krp.cli.internal.KrpApi
...
internal class KrpCommand : CliktCommand(name = ManifestUtils.appName, invokeWithoutSubcommand = true) {
    ...
    private val cmdServerAddr: Pair<String, Int> by option("-s", "--server-addr", help = "Server address, default: 127.0.0.1:7080")
        .convert { it.asAddr }
        .default("127.0.0.1" to 7080)

    private val cmdLocalAddr: Pair<String, Int> by option("-l", "--local-addr", help = "Local address, default: 127.0.0.1:80")
        .convert { it.asAddr }
    ...
    private val cmdLogFile: String? by option("--log-file", help = "Save log to file, e.g. krp.log")
    ...
    private val ini: Ini by lazy { Ini().also { it.load(File(cmdConfig ?: "krp.ini")) } }
    ...
    private fun startupKrp() {
        val basic = ini["basic"] ?: return
        val workerThreads = basic["worker_threads"].asInt() ?: -1
        val serverAddr: Pair<String, Int> = basic["server_addr"]?.asAddr ?: ("127.0.0.1" to 7080)
        val serverSslAddr = basic["server_ssl_addr"]?.asAddr ?: ("127.0.0.1" to 7443)
        val sslContext = runCatching {
            SslContextUtils.forClient(
                jks = basic["ssl_jks"] ?: "krp.jks",
                storePassword = basic["ssl_store_password"] ?: "krppass",
            )
        }.getOrNull() ?: SslContextUtils.forBuiltinClient()
        val httpApiBindAddr = basic["bind_http_api_addr"]?.asAddr
    ...
    private fun getTcpTunnelRequest(basic: Profile.Section, tunnel: Profile.Section): TunnelRequest {
        return TunnelRequest.forTcp(
            localIp = tunnel["local_addr"] ?: localIpV4 ?: "127.0.0.1",
