...
import krp.krp.utils.*

internal class KrpTunnelDaemonChannelHandler(
        ...
        when (msg) {
            ...
            ProtoMsgForceOff -> {
                ...
                ctx.channel().attr(AK_CHANNEL_INACTIVE_EXTRA).set(
                    ChannelInactiveExtra(true, KrpException("ForceOff"))
                )
            ...
            is ProtoMsgResponse -> {
                if (msg.status) {
                    ...
                    ctx.channel().attr(AK_TUNNEL_REQUEST).set(null)
                    ctx.channel().attr(AK_CHANNEL_INACTIVE_EXTRA).set(
                        ChannelInactiveExtra(false, KrpException(msg.payload))
                    )
                    ctx.channel().close()
            ...
            is ProtoMsgRemoteConnected -> {
                ...
                if (conn != null) {
                    krpListener?.onRemoteConnected(conn)
                }
                val tunnelRequest = ctx.channel().attr(AK_TUNNEL_REQUEST).get()
            ...
            is ProtoMsgRemoteDisconnect -> {
                val conn = runCatching { RemoteConnection.fromJson(msg.payload) }.getOrNull()
                if (conn != null) {
                    krpListener?.onRemoteDisconnect(conn)
                }
