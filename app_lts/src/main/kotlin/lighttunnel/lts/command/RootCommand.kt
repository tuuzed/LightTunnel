...
internal class RootCommand : CliktCommand(name = ManifestUtils.appName) {
    ...
    private fun configureLogger() {
        ...
        if (isStartFromCommand) {
            ...
            level = Level.toLevel(configJson.getOrNull<String>("log_level"), Level.INFO)
            file = configJson.getOrNull("log_file")
            maxBackupIndex = configJson.getOrNull("log_count") ?: 3
            maxFileSize = configJson.getOrNull("log_size") ?: "1MB"
        }
    ...
    private fun startupServer() {
        ...
        if (isStartFromCommand) {
            ...
            bossThreads = configJson.getOrNull<Int>("boss_threads") ?: -1
            workerThreads = configJson.getOrNull<Int>("worker_threads") ?: -1

            bindAddr = configJson.getOrNull<String>("bind_addr")?.let { IpUtils.parseAddr(it) } ?: ("0.0.0.0" to 7080)
            bindSslAddr = configJson.getOrNull<String>("bind_ssl_addr")?.let { IpUtils.parseAddr(it) }

            tunnelSslContext = runCatching {
                SslContextUtils.forServer(
                    jks = configJson.getOrNull("https_jks") ?: "lts.jks",
                    storePassword = configJson.getOrNull("https_store_password") ?: "ltspass",
                    keyPassword = configJson.getOrNull("https_key_password") ?: "ltspass",
                )
            }.getOrNull() ?: SslContextUtils.forBuiltinServer()

            bindHttpAddr = configJson.getOrNull<String>("bind_http_addr")?.let { IpUtils.parseAddr(it) }
            bindHttpsAddr = configJson.getOrNull<String>("bind_https_addr")?.let { IpUtils.parseAddr(it) }
            ...
            bindHttpApiAddr = configJson.getOrNull<String>("bind_http_api_addr")?.let { IpUtils.parseAddr(it) }
            httpApiUsername = configJson.getOrNull("http_api_username")
            httpApiPassword = configJson.getOrNull("http_api_password")
            ...
            httpPlugin = StaticFileHttpPlugin(
                paths = configJson.getOrNull<String>("plugin_sf_paths")?.split(",")?.toList() ?: emptyList(),
                hosts = configJson.getOrNull<String>("plugin_sf_hosts")?.split(",")?.toList() ?: emptyList()
            )
        }
        Server(
